...
public class pnlTimeKeepingManagment extends JPanel
	...
	private static final Logger LOGGER = Logger.getLogger(pnlTimeKeepingManagment.class.getName());
...
//		ArrayList<Account> dSPB = DepartmentDAO.getAllDePartment();
		...
		for (Timekeeping t : list) {
			tableModel2.addRow(
					new Object[] { tbl.getRowCount() + 1, new SimpleDateFormat("dd-MM-yyyy").format(t.getDate()),
							...
							t.getOverTime() + " Phút", t.getTotalHours() + " Phút", t.getStatus() });
		}
	}
