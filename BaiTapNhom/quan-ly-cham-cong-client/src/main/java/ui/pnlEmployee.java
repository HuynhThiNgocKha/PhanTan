...
//		Custom.restrictToNumbers(txtBS.getDocument());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnAdd)) {
			if (btnAdd.getText().trim().equals("Thêm")) {
				...
				if (employeeCount < 9) {
					builder.append("000").append(employeeCount + 1);
				} else if (employeeCount < 99) {
					builder.append("00").append(employeeCount + 1);
				}
	...
	private void deleteAction() {
		if (txtID.getText().equals(Controller.getAccount().getEmployee().getId())) {
			JOptionPane.showMessageDialog(this, "Bạn không thể xóa chính bạn!", null,
	...
	private void addAction() {
		try {
			Employee e = new Employee(txtID.getText(), txtName.getText(), txtDoB.getDate(), (radM.isSelected() ? true : false),
					txtAddress.getText(), txtPN.getText(), txtEmail.getText(), txtDoW.getDate(), new BigDecimal(txtBS.getText()),
					true, cmbRoles.getSelectedItem().toString(), false, (Department) cmbDepartments.getSelectedItem());
	...
	private void updateAction() {
		...
		try {
//			employeeDAO.updateEmployee(nv);
//			UpdateTabble(employeeDAO.getAll());
...
//			System.out.println(role);
	...
	public void mouseClicked(MouseEvent e) {
		...
		int row = tblTabble.getSelectedRow();
